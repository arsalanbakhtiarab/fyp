...
class NewsSentimentAnalyzer:
    ...
    def get_sentiment(self, headline):
        # Assuming estimate_sentiment function returns (probability, sentiment)
        probability, sentiment = estimate_sentiment([headline])
        return probability, sentiment
    
    def get_suggestion(sentiment, probability):
        if probability > 0.75:  # Adjust threshold as needed
            if sentiment == "positive":
                ...
                return "*Hold*"  # Fallback if sentiment is neutral but probability is high
        else:
            return "*Hold*"  # Default suggestion if probability is not high enough
    ...
    async def on_news(self, news):
        ...
        message = (f"*Company:* {self.symbol}\n*Date:* {formatted_date}\n*Time:* {formatted_time}\n"
                   f"*Real-time News:*\n{headline}\n"
                   f"*Real-time Sentiment:* {sentiment_info[1]} (*Probability:* {sentiment_info[0]})\n"
                   f"*Suggestion:* {suggestion}")
...
# Example usage
group_id = "JfVqbe4ZepL4N8X59LmtmO"  # Replace with your group's ID
