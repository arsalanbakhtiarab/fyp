...
def display_image_with_value(uptrend, downtrend, uptrend_value, downtrend_value):
    ...
    with col1:
        ...
        # st.markdown(f"<div style='position:relative; text-align:center; top:-25px; color:white;'>{uptrend_value}</div>",unsafe_allow_html=True)

    with col2:
        st.image(downtrend, width=width, caption='Downtrend', use_column_width=True)
        st.markdown("<style>div.row-widget.stHorizontal { padding-top: 30px; margin-bottom: -20px;}</style>", unsafe_allow_html=True)
        # st.markdown(f"<div style='position:relative; text-align:center; top:-25px; color:white;'>{downtrend_value}</div>",unsafe_allow_html=True)
